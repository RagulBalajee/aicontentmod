...
def analyze_image(image_path):
    ...
    print(f"SightEngine Raw Response: {data}") # Debugging line to see the full response
    ...
    nudity_raw = data.get("nudity", {}).get("raw", 0)
    nudity_partial = data.get("nudity", {}).get("partial", 0)

    # ✅ Significantly lowered thresholds to catch unsafe content with low confidence
    if gore_prob > 0.01 or data.get("gore", {}).get("classes", {}).get("very_bloody", 0) > 0.01:
        return {"status": "unsafe", "reason": "Gore Detected", "confidence": f"{gore_prob:.2%}"}
    if violence_prob > 0.001 or data.get("violence", {}).get("classes", {}).get("physical_violence", 0) > 0.001:
        return {"status": "unsafe", "reason": "Violence Detected", "confidence": f"{violence_prob:.2%}"}
    if weapon_prob > 0.001 or data.get("weapon", {}).get("classes", {}).get("firearm", 0) > 0.001:
        return {"status": "unsafe", "reason": "Weapon Detected", "confidence": f"{weapon_prob:.2%}"}
    if nudity_prob > 0.001 or nudity_raw > 0.001 or nudity_partial > 0.001:
        return {"status": "unsafe", "reason": "Inappropriate Content Detected", "confidence": f"Raw: {nudity_raw:.2%}, Partial: {nudity_partial:.2%}"}
