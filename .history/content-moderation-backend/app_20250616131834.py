...
def analyze_image(image_path):
    ...
    if gore_prob > 0.2 or data.get("gore", {}).get("classes", {}).get("very_bloody", 0) > 0.3:
        return {"status": "unsafe", "reason": "Gore Detected", "confidence": f"{gore_prob:.2%}"}
    if violence_prob > 0.2 or data.get("violence", {}).get("classes", {}).get("physical_violence", 0) > 0.3:
    ...
    if nudity_prob > 0.3 or nudity_raw > 0.1 or nudity_partial > 0.3:
